...
export default function Login() {
  ...
  if (user && !isLoading) {
    const roleMap: Record<string, string> = {
      "ROLE_EMPLOYEE": "employee/profile",
      "ROLE_HR_ADMIN": "hr_admin/employees",
      "ROLE_FINANCE_ADMIN": "finance_admin/payroll",
      "ROLE_MANAGER": "hr_admin/recruitment",
      "ROLE_SUPER_ADMIN": "super_admin/settings",
    };
    const firstRole = user.roles[0];
    const mappedPath = roleMap[firstRole] || "employee/profile";
    navigate(`/portal/${mappedPath}`, { replace: true });
    return null;
  }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        const mappedPath = roleMap[roleKey] || "employee/profile";
        navigate(`/portal/${mappedPath}`);
      }
    } catch (err) {
  ...
  const quickLogin = async (account: (typeof DEMO_ACCOUNTS)[0]) => {
    try {
      ...
      const roleMap: Record<string, string> = {
        ...
        "ROLE_HR_ADMIN": "hr_admin/employees",
        "ROLE_FINANCE_ADMIN": "finance_admin/payroll",
        "ROLE_MANAGER": "hr_admin/recruitment", // Default to recruitment or create specific dashboard
        "ROLE_SUPER_ADMIN": "super_admin/settings",
      };
      if (user) {
        const roleKey = user.roles[0];
          ...
          <div className="w-full max-w-6xl">
            ...
            <div className="text-center mb-12">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <h1 className="text-4xl font-bold text-foreground">HRMSuite</h1>
              </div>
                  ...
                  {DEMO_ACCOUNTS.map((account) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isLoading}
                        className="group relative overflow-hidden rounded-lg border border-border bg-card p-6 transition-all hover:border-primary hover:bg-muted/80 hover:shadow-lg hover:shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                  ...
                  <CardContent className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        disabled={isLoading}
                        className="bg-background border-input text-foreground placeholder:text-muted-foreground disabled:opacity-50"
                      />
                    </div>
